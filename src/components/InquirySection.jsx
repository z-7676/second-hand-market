    ...
    <div className="bg-white rounded-2xl border border-gray-100 p-8 shadow-sm animate-slide-up">
      ...
      {inquiries.length > 0 ? (
        <div className="space-y-4 mb-8 max-h-96 overflow-y-auto">
          {inquiries.map((inq) => (
            <div key={inq.id} className="p-4 bg-gray-50 rounded-xl">
              <div className="flex items-start justify-between mb-2">
                <span className="text-sm font-medium text-gray-700">{inq.author}</span>
                <span className="text-xs text-gray-400">{timeAgo(inq.time)}</span>
              </div>
                ...
                <div className="space-y-2 mb-3 pl-4 border-l-2 border-gray-200">
                  {inq.replies.map((reply, i) => (
                    <div key={i} className="text-sm">
                      ...
                      <span className="text-gray-500">{reply.message}</span>
                    </div>
                  ))}
                </div>
              )}
              ...
              {replyTo === inq.id ? (
                <div className="flex gap-2">
                  <input
                    ...
                    placeholder="回复内容..."
                    className="input-minimal flex-1 px-3 py-2 bg-white border border-gray-200 rounded-lg text-sm"
      ...
      <div className="border-t border-gray-100 pt-6">
        <div className="flex gap-3 mb-3">
          <input
            ...
            placeholder="你的昵称"
            className="input-minimal px-4 py-2 bg-gray-50 border border-gray-200 rounded-xl text-sm w-36"
          />
          <input
            value={message}
            onChange={(e) => setMessage(e.target.value)}
            placeholder="有什么想问的？关于成色、价格、交易方式..."
            className="input-minimal flex-1 px-4 py-2 bg-gray-50 border border-gray-200 rounded-xl text-sm"
            onKeyDown={(e) => e.key === 'Enter' && handleSubmitInquiry(e)}
          />
          <button
            onClick={handleSubmitInquiry}
            disabled={!message.trim() || !author.trim()}
            className="px-5 py-2 bg-gray-900 text-white rounded-xl text-sm font-medium hover:bg-gray-800 disabled:bg-gray-300 disabled:cursor-not-allowed transition-colors cursor-pointer"
          >
            发送
          </button>
        </div>
